...
reserved = {"39053344705", "52998224725", "11144477735", "16899535009"}
while True:
    n = [random.randint(0, 9) for _ in range(9)]
    n.append(dv(n, 9, 10)); n.append(dv(n, 10, 11))
    s = "".join(map(str, n))
    if len(set(s)) > 1 and s not in reserved:
        print(s); break
PY2
}

PLANO_INTERMEDIARIO="11111111-1111-1111-1111-111111111103"
...
line "Dois clientes novos com vaga no plano"
CPF_A=$(cpf_novo)
CPF_B=$(cpf_novo)
req POST /api/clientes "$ADMIN" "{\"tipoPessoa\":\"PF\",\"planoId\":\"$PLANO_INTERMEDIARIO\",\"nome\":\"E2E Multi A\",\"cpf\":\"$CPF_A\"}"
check 201 "$CODE" "criar cliente A"
CLIENTE_A=$(echo "$BODY" | jq -r '.id')
req POST /api/clientes "$ADMIN" "{\"tipoPessoa\":\"PF\",\"planoId\":\"$PLANO_INTERMEDIARIO\",\"nome\":\"E2E Multi B\",\"cpf\":\"$CPF_B\"}"
check 201 "$CODE" "criar cliente B"
CLIENTE_B=$(echo "$BODY" | jq -r '.id')
echo "  A=$CLIENTE_A B=$CLIENTE_B"
if [ -z "$CLIENTE_A" ] || [ -z "$CLIENTE_B" ] || [ "$CLIENTE_A" = "null" ] || [ "$CLIENTE_B" = "null" ]; then
  echo "FAIL [clientes de teste]"
...
if [ -n "$USER_ID" ] && [ "$USER_ID" != "null" ] && [ "$ID_B" = "$USER_ID" ]; then
  echo "PASS [mesma identidade]"; PASS=$((PASS+1))
else
  echo "FAIL [id diferente] $ID_B vs $USER_ID"; FAIL=$((FAIL+1))
fi
...
if [ -n "$TOKEN" ] && [ "$TOKEN" != "null" ]; then echo "PASS [login multi]"; PASS=$((PASS+1)); else echo "FAIL [login multi] body=$LOGIN_JSON"; FAIL=$((FAIL+1)); fi
if [ "$N_CLIENTES" = "2" ]; then echo "PASS [2 vinculações no login]"; PASS=$((PASS+1)); else echo "FAIL [clientes=$N_CLIENTES] $LOGIN_JSON"; FAIL=$((FAIL+1)); fi
...
if [ -n "$ROLE" ] && [ -n "$ROLE2" ] && [ "$ROLE" != "$ROLE2" ]; then
  echo "PASS [perfil muda conforme o cliente]"; PASS=$((PASS+1))
else
  echo "FAIL [perfil não mudou] $ROLE -> $ROLE2"; FAIL=$((FAIL+1))
fi
if [ -n "$OUTRO" ] && [ "$CID2" = "$OUTRO" ]; then
  echo "PASS [cliente_id do JWT atualizado]"; PASS=$((PASS+1))
else
  echo "FAIL [cliente_id=$CID2 esperado=$OUTRO]"; FAIL=$((FAIL+1))
fi
